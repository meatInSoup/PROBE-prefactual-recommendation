...
def similar_cost_heuristics(A_opt, x_0, data, A_0, epsilon, prev, cost_type="mahalanobis", w=None):
    l = data.shape[0]
    s = np.zeros(l)
    d = {}
    for i in range(l):
    ...
    for i in range(l - 1):
        ...
        norm_M_ij = np.linalg.norm(M_ij)
        if norm_M_ij == 0:
            print("Zero norm detected for M_ij. Skipping computation.")
            obj = 0
        elif np.any(np.isnan(M_ij)) or np.any(np.isinf(M_ij)):
            print("Invalid values detected in M_ij. Skipping computation.")
            obj = 0
        else:
            obj = np.abs(np.sum(np.multiply(A_opt, M_ij))) / norm_M_ij
        # obj = np.abs(np.sum(np.multiply(A_opt, M_ij))) / np.linalg.norm(M_ij) 
    ...
    for value in d_obj_sorted:
        if d_obj_sorted[value] not in prev:
            ...
            elif cost_type == "weighted-l1":
                data_i = np.reshape(data[d_obj_sorted[value][0]], x_0.shape)
                data_j = np.reshape(data[d_obj_sorted[value][1]], x_0.shape)
                l1 = np.sum(w * np.abs(data_i - x_0))
                l2 = np.sum(w * np.abs(data_j - x_0))
                obj = l1 - l2
            res = M_ij if obj <= epsilon else -M_ij
            return res, d_obj_sorted[value], (data[d_obj_sorted[value][0]], data[d_obj_sorted[value][1]])


def similar_cost_heuristics_k(A_opt, x_0, data, A_0, epsilon, prev, k=4, cost_type="mahalanobis", w=None):
    l = data.shape[0]
    s = np.zeros(l)
    d = {}
    for i in range(l):
    ...
    for i in range(len(min_l)):
        ...
        if cur_l not in prev:
            if cost_type == "mahalanobis":
                ...
                cost_l = [np.sum(w * np.abs(data[d_list[i]] - x_0)) for i in cur_l]
            ...
            for i in cur_l:
                if i != min_idx:
                    ...
                    if cost_type == "mahalanobis":
                        ...
                        l1 = np.sum(w * np.abs(data[d_list[min_idx]] - x_0))
                        l2 = np.sum(w * np.abs(data[d_list[i]] - x_0))
                        obj = l1 - l2
                    res = M_ij if obj <= epsilon else -M_ij
                    M_ij_l.append(res)
...
def similar_cost_heuristics_kpairs(A_opt, x_0, data, k=4, cost_type="mahalanobis"):
        """
        :type nums: List[int]
        :type k: int
        :rtype: int
...
def find_q(x_0, data, T, A_0, epsilon, cost_correction, pair=True, cost_type='mahalanobis', w=None):
    ...
    for i in range(T):
        ...
        # 1 question
        if pair == True:
            M_ij, pair_index, pair_data = similar_cost_heuristics(A_opt, x_0, data, A_0, epsilon, prev, cost_type, w)
            prev.append(pair_index)
            P.append(M_ij)
        
        # k questions
        else:
            M_ij_l, pairs = similar_cost_heuristics_k(A_opt, x_0, data, A_0, epsilon, prev, k=4, cost_type=cost_type, w=w)
            prev.append(pairs)
            P += M_ij_l
