...
def generate_recourse(x_0, model, params=dict()):
    ...
    post_Sigma, post_m, log_dict = question_scheme.bayesian_PE_diag(params["A"], prior_Sigma, prior_m, x_0, feasible_set, sessions, iterations, lr, tau=TAU, size=size, pair=False, cost_type=cost_type, w=params['w'])

    # Graph-based recourse generation
    graph_opt = graph.bayesian_build_graph(train_data, post_Sigma, post_m, n_neighbors, diag=True)
